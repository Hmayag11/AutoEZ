...
%% open the MRI in 3D. THIS FILE WAS GENERATED BY BRAINSTORM
...
temporal_map_new=[temporal_map_new;mean(temporal_map)>1.7];
end
...
for bd=1:7;
    network=networks_new{bd};%
    for j=1:2 %background and epil
    U=network(j,:).*(network(j,:)>0.3);%%mean(network(j,:))+std(network(j,:)))
...
bd_current=2; %% choose band  2=theta
x=[1 2];
legends={'EN','BN'};
